...
# wall detect and can return
    ...
    execute store success storage storage:booleans Main.Weapons.Magics.Rods.Shot.Retrun byte 1 run kill @s[tag=Magic.Shot.DetectedWall]
...
# scope over
    ...
    execute store success storage storage:booleans Main.Weapons.Magics.Rods.Shot.Retrun byte 1 if score $Magic.Shot.NowScope counter >= #Magic.Shot.ScopeLimit temporary
