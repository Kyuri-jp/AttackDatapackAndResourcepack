...
# result
    ...
    #error
    execute unless entity @e[type=marker,tag=Pis.Marker,tag=Pis.Marker.Here] run data merge storage error:info {Level:error,Path:"libs:player_item_storage/entity/same_uuid",Message:"UUIDが一致するMarkerが見つかりませんでした",StackTrace:"UUID matched Marker not found."}
