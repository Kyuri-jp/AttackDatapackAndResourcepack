# retrun
    execute unless entity @e[type=armor_stand,tag=storage.players.items.marker.here,limit=1] run tellraw @a ["",{"text":"[Error](libs:players/player_item_storage/inventory/summon)","bold":true,"color":"dark_red"},{"text":"エンティティが存在しません (not found storage.players.items.marker.here)"}]
...
# get items
    ## summon
        summon item ~ ~ ~ {Tags:["storage.players.items.generated"],Item:{id:"minecraft:stick",Count:1,tag:{display:{Name:'{"text": "Item Of Null"}'}}},PickupDelay:0}
