...
bossbar set attack.game.datas name [{"text":"[Exhibition Game]Time remaining:"},{"score":{"name":"$attack.timer.exGame.sec","objective":"datas"}},{"text":" seconds"}]
execute store result bossbar attack.game.datas value run scoreboard players get $attack.timer.exGame datas
execute if score $attack.timer.exGame datas matches ..0 run function systems:game/end/judgment/detect
execute unless score $attack.timer.exGame datas matches ..0 run schedule function systems:game/ex/start_datas 1t
